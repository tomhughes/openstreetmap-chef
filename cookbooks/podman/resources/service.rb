...
action :create do
  systemd_service new_resource.service do
    ...
    exec_start "/usr/bin/podman run --cidfile=%t/%n.ctr-id --cgroups=no-conmon --userns=auto --label=io.containers.autoupdate=registry --network=slirp4netns:mtu=1500 #{publish_options} #{volume_options} #{environment_options} --rm --sdnotify=conmon --detach --replace --name=%N #{new_resource.image}"
...
action_class do
  ...
  def volume_options
    new_resource.volumes.collect do |host, guest|
      "--volume=#{host}:#{guest}"
    end.join(" ")
  end
